...
BUILD_HARNESS_VERSION := 2.0.12
# renovate: datasource=docker depName=ghcr.io/defenseunicorns/packages/dubbd-k3d extractVersion=^(?<version>\d+\.\d+\.\d+)
DUBBD_K3D_VERSION := 0.21.0
...
build/all: build build/zarf build/zarf-init build/dubbd-k3d build/uds-capability-jenkins ##
...
build/uds-capability-jenkins: | build ## Build the jenkins capability
...
deploy/all: deploy/init deploy/dubbd-k3d deploy/uds-capability-jenkins ##
...
deploy/uds-capability-jenkins: ## Deploy the jenkins capability
	cd build && ./zarf package deploy zarf-package-jenkins-*.tar.zst --confirm
...
.PHONY: all
all: build/all cluster/reset deploy/all ## Build and deploy jenkins locally
